...
// AICronTaskLogChatIDs returns the chat ids referenced by a task's execution
// logs. Used by the task-delete handler so it can clean up the per-run chats
// alongside the logs; must be called before the logs are deleted.
func AICronTaskLogChatIDs(c *ctx.Context, taskId int64) ([]string, error) {
	var rows []AICronTaskLog
	err := DB(c).Where("task_id = ?", taskId).Select("chat_id").Find(&rows).Error
	if err != nil {
		return nil, err
	}
