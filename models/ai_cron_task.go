...
type AICronTask struct {
	...
	Name        string  `json:"name" gorm:"type:varchar(255);not null;default:'';uniqueIndex:idx_name"`
...
// AICronTaskSetLastStatus mirrors an execution log's terminal status onto the
// task row. The dispatcher optimistically writes "success" once a run is handed
// to the async agent; this update reconciles that with the real terminal result.
func AICronTaskSetLastStatus(c *ctx.Context, id int64, status string) error {
	return DB(c).Model(&AICronTask{}).Where("id = ?", id).UpdateColumn("last_status", status).Error
}
