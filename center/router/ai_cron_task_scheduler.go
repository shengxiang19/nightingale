...
func (s *AICronScheduler) Add(task *models.AICronTask) {
	...
	entryId, err := s.cron.AddFunc(task.CronExpr, func() {
		...
		// Cron-triggered runs have no acting user, so the chat is owned by the
		// task creator (the account under whose sidebar the result is grouped).
		owner, uerr := models.UserGetByUsername(s.ctx, fresh.CreatedBy)
		if uerr != nil || owner == nil {
			logger.Warningf("[AICron] task %d owner %q not found, skip run: %v", fresh.Id, fresh.CreatedBy, uerr)
			return
		}
		if _, _, _, rerr := s.rt.runAICronTask(fresh, owner.Id); rerr != nil && !errors.Is(rerr, ErrAICronTaskBusy) {
...
// task. Per-run chats (instead of one accumulating per-task chat) let 「查看结果」
// point each execution history row to its own conversation, and let the sidebar
// group them under the task as a folder. TaskId/TaskName mark the chat so it is
// excluded from the normal chat history and aggregated under the sidebar 「任务」
// section; TaskName is denormalized so an execution chat can be labeled even
// after its task is deleted. The chat is owned by the user who triggered the
// run (ownerUserID): the task creator for cron-triggered runs, or the acting
// user for a manual run-now.
func (rt *Router) aiCronNewRunChat(task *models.AICronTask, ownerUserID int64) (*models.AssistantChat, error) {
...
func (rt *Router) runAICronTask(task *models.AICronTask, ownerUserID int64) (chatId string, seqId int64, logId int64, err error) {
	s := rt.getAICron()
	...
	chat, err := rt.aiCronNewRunChat(task, ownerUserID)
	if err != nil {
		return "", 0, 0, err
	}
	chatId = chat.ChatID
	...
	result, status, err := rt.StartAssistantMessage(chat.UserID, chat, models.AssistantMessageQuery{Content: task.Content}, "", &AssistantRunOverride{
